...
<x-app-layout>
            ...
            <template x-if="files.length > 0">
              <ul class="mt-4 space-y-2">
                <template x-for="file in files" :key="file.name">
                  <li class="flex items-center justify-between rounded-lg bg-gray-100 px-4 py-2 dark:bg-gray-200">
                    <span class="truncate text-sm font-medium text-gray-700" x-text="file.name"></span>
  ...
  <div class="flex w-full items-center justify-between">
    <div class="w-[50%]">
      <label for="summary-name" class="block text-sm/6 font-medium text-fg-secondry dark:text-fg-secondry-dark">Summary
        title</label>
      <div class="mt-2">
    ...
    <div x-data="{
      ...
      <div class="relative mt-2">
        ...
        <ul x-show="open" x-cloak
          class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-bg-primary py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none dark:bg-bg-primary-dark sm:text-sm"
          ...
          @forelse ($folders as $folder)
            <li
              ...
              class="relative cursor-pointer select-none py-2 pl-3 pr-9 text-fg-secondry hover:bg-bg-active hover:text-fg-primary dark:text-fg-secondry-dark dark:hover:bg-bg-active-dark"
              role="option">
              <div class="flex items-center">
                <div class="h-4 w-4 rounded-[4px]" style="background-color: {{ $folder->color }};"></div>
                <span class="ml-3 block truncate">{{ $folder->name }}</span>
              </div>
            </li>
          @empty
            <li class="relative cursor-default select-none py-2 pl-3 pr-9 text-fg-secondry dark:text-fg-secondry-dark"
              role="option">
              <div class="flex items-center">
                <span class="ml-3 block truncate">No folders available</span>
              </div>
            </li>
          @endforelse
        </ul>
  ...
  <div class="w-full">
    <button type="submit" class="btn w-full" :disabled="loading">
      ...
      <progress class="progress w-56" x-show="loading" x-cloak></progress>
    </button>
  </div>
